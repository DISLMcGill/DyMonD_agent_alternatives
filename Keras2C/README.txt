Keras2C:
-------------------------
** in keras2c_example, all the steps are done -- can skip to any step

Training:
Run training_in_python.py to obtain 5 .h5 models. This script will also test each model, and generate performance metrics on the test data in TestNoSLCor.csv. The data the model is trained on is in TrainNoSLCor.csv: each line is a series of test data, and the corresponding correct prediction. Select the model with the f1 score closest to the average of the 5 models.
 
Testing:
1) Using the automatic test suite:
...
2) To create and run a test suite to generate predictions from tests in TestNoSLCor.csv, and compare to real values:
